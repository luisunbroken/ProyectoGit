...
class MrpProduction(osv.Model):
    ...
    def create(self, cr, uid, values, context=None):
        ...
        if 'product_id' in values:
            product = self.pool.get('product.product').browse(cr, uid, values.get('product_id'), context=context)
            if product.categ_id:
                if product.categ_id.location_src_id:
                    values['location_src_id'] = product.categ_id.location_src_id.id
                if product.categ_id.location_dest_id:
                    values['location_dest_id'] = product.categ_id.location_dest_id.id                
        return super(MrpProduction, self).create(cr, uid, values, context=context)